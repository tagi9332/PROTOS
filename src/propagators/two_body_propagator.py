...
def step_2body(state: dict, dt: float, config: dict):
    ...
    def compute_accel(r: np.ndarray, perturb_config: dict):
        """
        Compute total acceleration: central body + perturbations
        """
        r_mag = np.linalg.norm(r)
        a_total = -MU_EARTH * r / r_mag**3
        a_total += compute_perturb_accel(r, perturb_config)
        return a_total

    # RK4 integration for chief
    k1_vc = compute_accel(chief_r, perturb_config) * dt
    k1_rc = chief_v * dt

    k2_vc = compute_accel(chief_r + 0.5 * k1_rc, perturb_config) * dt
    k2_rc = (chief_v + 0.5 * k1_vc) * dt

    k3_vc = compute_accel(chief_r + 0.5 * k2_rc, perturb_config) * dt
    k3_rc = (chief_v + 0.5 * k2_vc) * dt

    k4_vc = compute_accel(chief_r + k3_rc, perturb_config) * dt
    ...
    k1_vd = compute_accel(deputy_r, perturb_config) * dt
    k1_rd = deputy_v * dt

    k2_vd = compute_accel(deputy_r + 0.5 * k1_rd, perturb_config) * dt
    k2_rd = (deputy_v + 0.5 * k1_vd) * dt

    k3_vd = compute_accel(deputy_r + 0.5 * k2_rd, perturb_config) * dt
    k3_rd = (deputy_v + 0.5 * k2_vd) * dt

    k4_vd = compute_accel(deputy_r + k3_rd, perturb_config) * dt
